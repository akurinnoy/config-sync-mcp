...
describe('Auth-enabled server', () => {
  ...
  it('returns 401 for unauthenticated POST /mcp when auth enabled', async () => {
    mockAuthModule();
    const { startHttpServer, shutdownHttpServer } = await import('../../src/server.js');
    ...
    const testConfig: any = {
      profiles: [],
      storage: { healthCheck: async () => ({ healthy: true }), initialize: async () => {} },
      homeDir: '/tmp',
      resolver: { resolve: async () => ({}) },
    };

    const authServer = await startHttpServer(0, testConfig, authMiddleware);
    const authPort = (authServer.address() as any).port;

    try {
      const res = await new Promise<{ status: number; body: any }>((resolve, reject) => {
        const req = http.request(
          { hostname: '127.0.0.1', port: authPort, path: '/mcp', method: 'POST', headers: { 'Content-Type': 'application/json' } },
          (res) => {
            const chunks: Buffer[] = [];
            res.on('data', (c) => chunks.push(c));
      ...
      await shutdownHttpServer(authServer);
    }
  });

  it('allows GET /healthz without auth when auth enabled', async () => {
    mockAuthModule();
    const { startHttpServer, shutdownHttpServer } = await import('../../src/server.js');
    const { rawHttpK8sAuth, createDefaultK8sClient } = await import('@che-incubator/k8s-mcp-auth');
    ...
    try {
      const res = await new Promise<{ status: number; body: any }>((resolve, reject) => {
        http.get(`http://127.0.0.1:${authPort}/healthz`, (res) => {
          const chunks: Buffer[] = [];
          res.on('data', (c) => chunks.push(c));
          res.on('end', () => {
            resolve({ status: res.statusCode!, body: JSON.parse(Buffer.concat(chunks).toString()) });
  ...
  it('passes through with valid bearer token', async () => {
    ...
    try {
      const res = await new Promise<{ status: number }>((resolve, reject) => {
        ...
        req.on('error', reject);
        req.end(JSON.stringify({ jsonrpc: '2.0', method: 'initialize', params: { protocolVersion: '2025-03-26', capabilities: {}, clientInfo: { name: 'test', version: '0.1.0' } }, id: 1 }));
      });
